...
void diez_pasos()
{
	int Gearbox_Status = 1; // 1 = Drive
	int Veh_Speed   = zero_km; 
	int Operation   = Veh_Speed;
	int Battery     = OFF;
	int Ign_Status  = ON;
	int Door_Status = 1;
	int Active_Door_Sensor = Door_Status; 
	int Toogle_seatbelt = 0;
	//int SeatBelt_Status = SealtBeltIndicator;   //0 = Turn_OFF, 1 = Turn_ON, 2 = Toogle
	...
	//Active_Door_Status = 1, puerta,2 cofre,3 cajuela, 0 cerrada 
	...
	{
		SIU.GPDO[1].B.PDO = TURN_ON;//door_Status
	}
	else if(Operation == 6){
		SIU.GPDO[1].B.PDO = TURN_OFF;//door_status
	}
	
	if((Gearbox_Status == 1) && (Operation == 0 || Operation == 1 || Operation == 2 || Operation == 3 || Operation == 4 || Operation == 5))
	{
		Toogle_seatbelt = 1;
		
		while(Toogle_seatbelt == 1 ){
			SIU.GPDO[4].B.PDO = TURN_ON;
			t(4000000);
		    SIU.GPDO[4].B.PDO = TURN_OFF;
		    t(9000000);
			
		}
	}
	if((Gearbox_Status == 1) && (Operation == 6)){
		SIU.GPDO[4].B.PDO = TURN_ON;
	}
