...
uint8_t acteng_msb = 0;  //B0
uint8_t acteng_lsb = 0;  //B1

uint8_t Increase_ACK = 0;  //MSB B0
uint8_t Odometer_Value = 0; //LSB B1

uint8_t TNK_MSB = 0;   //MSB B0
uint8_t TNK_LSB= 0;    // LSB  B1
uint8_t GAS_Status = 0; // B2

uint8_t Door_Status = 0;   //B0
uint8_t High_Beams_Status = 0; //B1
uint8_t HandBrake_Status = 0; //B2

uint8_t Seatbelt_Status = 0; //B0
uint8_t Ign_Status = 0;     //B0
uint8_t Gearbox_Status = 0; //B0
...
void Veh_Speed(CAN_MessageDataType CanMessage0)
{
	...
	Can_Vel_Message = CanMessage0; //  se recive CanMeessage que es el parametro de entrada de la funcion y se cambia por  canmessage put handler
...
void  Door_Status_Indicator(CAN_MessageDataType CanMessage3)
{
	//SIU.GPDO[69].B.PDO = 1; //DEJA PRENDIDO PIN1 PUERTO A COMO INDICADOR DE QUE SE RECIVIO EL ID
		Indicators_Message = CanMessage3; //  se recive CanMeessage uqe es el parametro de entrada de la funcion y se cambia por  canmessage put handler
				...
				if ((Door_Status == 7) &&  (High_Beams_Status== 8))
