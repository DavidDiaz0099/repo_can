...
void Veh_Speed(CAN_MessageDataType CanMessage0)
{
	...
	acteng_msb = 0;
	acteng_lsb = 0;
	acteng_msb = CanMessage0.msg_data_field[0]; //
	acteng_lsb = CanMessage0.msg_data_field[1]; //
...
void Odometer(CAN_MessageDataType CanMessage1)
{
	...
	//Can_Odo_Message = CanMessage1; //  se recive CanMeessage uqe es el parametro de entrada de la funcion y se cambia por  canmessage put handler
	   ...
	   Increase_ACK = 0;
	   Odometer_Value = 0;
	   Increase_ACK = CanMessage1.msg_data_field[0]; //
		Odometer_Value = CanMessage1.msg_data_field[1]; //
...
void  Door_Status_Indicator(CAN_MessageDataType CanMessage3)
{
		...
		Indicators_Message = CanMessage3; //  se recive CanMeessage uqe es el parametro de entrada de la funcion y se cambia por  canmessage put handler
				...
				//	SIU.GPDO[3].B.PDO = 1;  //DEJA PRENDIDO LED2 DE LA TARJETA COMO INDICADOR DE QUE SE RECIVIO EL ID	
			//msg_rx_pdu0 = 1;
			//PduHandlerCnt0++;	
}
...
///////////////////////////////////////////////////////////////////////////////////////////////
void Directionals(CAN_MessageDataType CanMessage7) //chida
{
	Directional_Message = CanMessage7;
	
	Directional_Status = Directional_Message.msg_data_field[0]; 
	Temp_high_Status = Directional_Message.msg_data_field[1];
	Gas_low_Status = Directional_Message.msg_data_field[2]; 
	Oil_low_Status = Directional_Message.msg_data_field[3];    //
	High_beams_Status = Directional_Message.msg_data_field[4]; //
	Hand_break_Status = Directional_Message.msg_data_field[5]; //
	Battery_low_Status = Directional_Message.msg_data_field[6];
	Door_open_Status = Directional_Message.msg_data_field[7];
}
...
void Gas_low(CAN_MessageDataType CanMessage9)
{
	...
	//Gas_low_Status = Gas_low_Message.msg_data_field[0]; 
	
	
	
}

void Gas_low_Luis(void)
{

	/*if ( 1== CAN_ReceiveFrame(0,&Gas_low_Message))
...
void Door_open(CAN_MessageDataType CanMessage14) //chida
{
	Door_open_Message = CanMessage14; 
	// = Door_open_Message.msg_data_field[0]; 
}
